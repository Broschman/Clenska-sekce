...
def check_password():
    """Vrátí True, pokud je uživatel přihlášen. Řeší cookies, roli i Tabulátorový selectbox."""
    
    # Inicializace manageru
    cookie_manager = stx.CookieManager(key="auth_cookie")

    # 1. DELEGOVANÝ VÝMAZ SUŠENKY: Tady se povel konečně propíše až do prohlížeče!
    ...
    # 2. Povolíme vstup těm, kteří už jsou přihlášeni (lokálně)
    if st.session_state.get("password_correct", False):
        return True

    # 3. AUTOMATICKÉ PŘIHLÁŠENÍ (Remember Me z Cookies)
    if cookie_hodnota:
        try:
            ...
            if not df_jmena[mask].empty:
                ...
                if str(ulozene_heslo).strip() == correct_pin:
                    # Naplnění kritických stavů pro app.py
...
def logout():
    """Odhlásí uživatele pomocí asynchronního delegování."""
    # 1. Okamžitě vymažeme paměť serveru
    for key in ["password_correct", "prihlaseny_uzivatel", "role"]:
        if key in st.session_state:
            del st.session_state[key]
            
    # 2. Nahodíme vlajku pro check_password, ať smaže sušenku v dalším cyklu
    st.session_state["do_logout"] = True
    
    # 3. Překreslíme celou obrazovku (díky tomu nezabijeme React komunikaci)
    st.rerun()
