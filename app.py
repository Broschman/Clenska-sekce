...
# --- HLAVNÍ JÁDRO: VYKRESLENÍ DETAILU AKCE ---
# Tato funkce se používá jak v kalendáři, tak v horním dashboardu
def vykresli_detail_akce(akce, unique_key, conn, seznam_jmen):
    dnes = date.today()
    je_po_deadlinu = dnes > akce['deadline']
    je_dnes_deadline = dnes == akce['deadline']
    
    typ_udalosti = str(akce['typ']).lower().strip() if 'typ' in akce and pd.notna(akce['typ']) else ""
    druh_akce = str(akce['druh']).lower().strip() if 'druh' in akce and pd.notna(akce['druh']) else "ostatní"
    ...
    elif "oblastní" in typ_udalosti: typ_label_short = "OBLASTNÍ"
    elif "štafety" in typ_udalosti: typ_label_short = "ŠTAFETY"
    ...
    with col_info:
        ...
        st.markdown(badge(typ_label_short, bg="#F3F4F6", color="#333") + badge(druh_akce.upper(), bg="#E5E7EB", color="#555"), unsafe_allow_html=True)
        ...
        kategorie_txt = str(akce['kategorie']).strip() if 'kategorie' in akce and pd.notna(akce['kategorie']) else ""
        if kategorie_txt:
            st.write(f"🎯 **Kategorie:** {kategorie_txt}")
        st.markdown("</div>", unsafe_allow_html=True)
        if pd.notna(akce['popis']): st.info(f"{akce['popis']}", icon="ℹ️")
        
        deadline_str = akce['deadline'].strftime('%d.%m.%Y')
        st.markdown("---")
        if je_po_deadlinu: st.error(f"⛔ **DEADLINE BYL:** {deadline_str}")
        elif je_dnes_deadline: st.warning(f"⚠️ **DNES JE DEADLINE!** ({deadline_str})")
        else: st.success(f"📅 **Deadline:** {deadline_str}")

        if je_zavod_obecne:
            st.caption("Přihlášky probíhají v systému ORIS.")
            odkaz_zavodu = str(akce['odkaz']).strip() if 'odkaz' in akce and pd.notna(akce['odkaz']) else ""
    ...
    # --- PRAVÝ SLOUPEC: FORMULÁŘ + SEZNAM ---
    with col_form:
        akce_id_str = str(akce['id']).replace('.0', '') if 'id' in akce else ""
        delete_key_state = f"confirm_delete_{unique_key}"
        
        # FORMULÁŘ (Vždy viditelný)
        with stylable_container(key=f"form_cont_{unique_key}", css_styles="{border: 1px solid #E5E7EB; border-radius: 12px; padding: 20px; background-color: #F9FAFB; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);}"):
            if not je_po_deadlinu and delete_key_state not in st.session_state:
                ...
                with st.form(key=form_key, clear_on_submit=True):
                    if kategorie_txt and kategorie_txt.lower() != "všichni": st.warning(f"Podmínka: **{kategorie_txt}**")
                    ...
                    with stylable_container(key=f"submit_btn_{unique_key}", css_styles="button {background-color: #16A34A !important; color: white !important; border: none !important; transform: translateY(-10px) !important; margin-top: 0px !important;} button:hover {background-color: #15803d !important; color: white !important;}"):
            ...
            elif je_po_deadlinu: st.info("🔒 Tabulka uzavřena.")
        ...
        except: df_prihlasky = pd.DataFrame()
        ...
        if delete_key_state in st.session_state:
            ...
            with st.container():
                ...
                if c_yes.button("✅ ANO", key=f"yes_{unique_key}"):
                    try:
                        mask = (df_prihlasky['id_akce'] == akce_id_str) & (df_prihlasky['jméno'] == clovek_ke_smazani)
                        df_clean = df_prihlasky[~mask]
        ...
        if not lidi.empty:
            ...
            for i, (idx, row) in enumerate(lidi.iterrows()):
                ...
                with stylable_container(key=f"row_{unique_key}_{idx}", css_styles=f"{{background-color: {bg_color}; border-radius: 6px; padding: {padding_style}; margin-bottom: 2px; display: flex; align-items: center; min-height: 40px;}}"):
                    ...
                    c5.write(str(row['ubytování']) if pd.notna(row.get('ubytování')) else "")
                    if not je_po_deadlinu:
                        with stylable_container(key=f"del_btn_cont_{unique_key}_{idx}", css_styles="button {margin: 0px !important; padding-top: 0px !important; padding-bottom: 0px !important; height: auto !important; min-height: 0px !important;}"):
                            if c6.button("🗑️", key=f"del_{unique_key}_{idx}"):
                                st.session_state[delete_key_state] = row['jméno']
                                st.rerun()
        else: st.caption("Zatím nikdo. Buď první!")

# --- CSS VZHLED (DESIGN 4.3 - STEALTH MODE + LOGO HEADER) ---
st.markdown("""
<style>
...
try:
    ...
    try:
        ...
        # Ošetření sloupců, kdyby neexistovaly
        if 'doprava' not in df_prihlasky.columns: df_prihlasky['doprava'] = ""
        if 'ubytování' not in df_prihlasky.columns: df_prihlasky['ubytování'] = ""
...
if not future_deadlines.empty:
    ...
    for i, (_, row) in enumerate(future_deadlines.iterrows()):
        ...
        unique_key_dash = f"dash_{row['id']}_{i}"
        
        with cols_d[i]:
            # Zde vytvoříme tlačítko pro dashboard
            with stylable_container(
                ...
                with st.popover(label_text, use_container_width=True):
                    # TADY VOLÁME NAŠI SDÍLENOU FUNKCI Z 1. ČÁSTI
                    vykresli_detail_akce(row, unique_key_dash, conn, seznam_jmen)
    ...
    for i, den_cislo in enumerate(tyden):
        with cols[i]:
            ...
            for _, akce in akce_dne.iterrows():
                ...
                elif "oblastní" in typ_udalosti or "žebříček" in typ_udalosti: style_key = "oblastni"
                elif "zimní liga" in typ_udalosti or "bzl" in typ_udalosti: style_key = "zimni_liga"
                elif "štafety" in typ_udalosti: style_key = "stafety"
                elif "trénink" in typ_udalosti: style_key = "trenink"
                elif je_zavod_obecne: style_key = "oblastni"
                ...
                with stylable_container(
                    ...
                    with st.popover(final_text, use_container_width=True):
                        # TADY VOLÁME NAŠI SDÍLENOU FUNKCI Z 1. ČÁSTI
                        vykresli_detail_akce(akce, unique_key, conn, seznam_jmen)
...
with stylable_container(
    ...
    with col_center:
        ...
        <div style='text-align: center; color: #9CA3AF; font-size: 0.8em; font-family: sans-serif;'>
            <b>Členská sekce RBK</b> • Designed by Broschman • v1.9<br>
