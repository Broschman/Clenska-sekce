                    ...
                    if novy_pin != potvrzeni:
                        ...
                        try:
                            ...
                            if not spravny_radek.empty:
                                # Oříznutí nezlomitelné mezery a případného apostrofu při čtení starého hesla
                                real_pin = str(spravny_radek.iloc[0].get(col_pin, '')).replace('\u00A0', '').lstrip("'").strip()
                                if real_pin.endswith('.0'): real_pin = real_pin[:-2]

                                if stary_pin.strip() == real_pin:
                                    # 1. Přetypování celého sloupce na text PŘED ZÁPISEM (Prevence float erroru)
                                    df_jmena[col_pin] = df_jmena[col_pin].astype(str).str.replace(r'\.0$', '', regex=True)
                                    
                                    # 2. Ošetření GSheets vzorců (přidání apostrofu)
                                    ciste_heslo = str(novy_pin).strip()
                                    if ciste_heslo.startswith(("+", "=", "-")):
                                        heslo_do_db = f"'{ciste_heslo}"
                                    else:
                                        heslo_do_db = ciste_heslo

                                    # 3. Zápis nového hesla
                                    idx = spravny_radek.index[0]
                                    df_jmena.at[idx, col_pin] = heslo_do_db
                                    
                                    # 4. Zápis do DB a reset cache
                                    ...
                                    # Uložení do cookies šifrovaně v Base64
                                    cookie_str = f"{prihlaseny}|{ciste_heslo}"
